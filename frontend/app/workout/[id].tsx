 ...
 * Loads/persists against the backend when given a real workout id; falls back
...
/** Map a server WorkoutExerciseOut (+ optional previous-session sets) to the local model. */
...
export default function ActiveWorkout() {
  ...
  /** Pull server truth into local state. No-op for the offline demo seed. */
  ...
  // Debounced per-field persistence (weight/reps text edits).
  ...
  useEffect(() => {
    ...
    // here — from the server's started_at, the same source the elapsed clock uses.
    if (startedAtRef.current == null) return;
    activityRunning.current = LiveActivity.start(startedAtRef.current, state) != null;
    // `startedAt` arrives with the server payload, after the first run — without
  ...
  const editWeight = (exId: string, setId: string, text: string) => {
    ...
    if (persist) {
      ...
      debounce(`w:${setId}`, () => write(patchSetApi(setId, { weight: Number.isNaN(w) ? null : w })));
    }
  };
  ...
  const editReps = (exId: string, setId: string, text: string) => {
    ...
    if (persist) {
      ...
      debounce(`r:${setId}`, () => write(patchSetApi(setId, { reps: Number.isNaN(r) ? null : r })));
    }
  };
  ...
  /**
   * Swap an exercise, keeping its position. The backend has no endpoint for
   * this, so it is add + remove + reorder: `addWorkoutExercise` appends, and
  ...
  applyReplace.current = async (exId, chosen) => {
    ...
    try {
      ...
      // Swallowed so the caller still refetches: whatever the server actually
  ...
  /** Commit a full reorder from the drag overlay; the server stores `position`. */
  ...
  /**
   * Tear down and leave a discarded workout. The server discard is awaited before
  ...
  const onFinish = async () => {
    ...
    // `startedAt` is the same server-derived origin the elapsed clock uses.
    if (startedAt != null) {
