...
struct CompleteSetIntent: LiveActivityIntent {
  ...
  func perform() async throws -> some IntentResult {
    // Redraws the card from `next`, which JS supplied. The real write — carry-
    // forward, persistence, starting the real rest — still happens in JS, so no
    // set-writing logic lives in the extension.
