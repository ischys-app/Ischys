...
/** Response of GET /workouts/summary/activity?weeks=N. */
...
export type ExerciseOut = {
  ...
  /** Server-relative path (`/media/...`); resolve with `mediaUrl(serverUrl, …)`. */
...
/** Mirrors backend `app.models.enums.WorkoutStatus`. */
...
/** One past session for a single exercise (from GET /exercises/{id}/history). */
...
/** Personal record metric slug used by the backend records endpoint. */
export type RecordMetric = 'best_set' | 'est_1rm' | 'best_volume' | 'max_reps';

/** One record card (from GET /exercises/{id}/records). */
export type RecordOut = {
  metric: RecordMetric;
  value: number;
  display: string;
  achieved_at?: string | null;
  /** Present on /records/recent so the Profile list can name the lift. */
  exercise_name?: string | null;
};

/** Chart series (from GET /exercises/{id}/chart). */
...
/** Response shape of GET /profile. Mirrors backend `ProfileOut` (schemas.py). */
...
/** One target set within a saved routine (position-ordered by the server). */
...
/** Full routine payload returned by GET /routines/{id}. */
...
/** Full server-persisted settings payload (GET /settings). */
...
/** Any subset — used as body for PATCH /settings. */
export type SettingsUpdate = Partial<SettingsOut>;

// --- Import ---

/**
 * Response of POST /api/v1/import?source=hevy. Mirrors backend
 * `ImportResult` (schemas.py) — counts of what landed after mapping the CSV.
 */
