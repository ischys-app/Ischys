...
export function DemoSlot({ exerciseId, initialUrl, fallbackImageUrl, imageAuthor }: Props) {
  ...
  // Rehydrate from SecureStore when the exercise row carries no URL.
  useEffect(() => {
    let cancelled = false;
  ...
  const saveUrl = async (next: string) => {
    ...
    // Nothing else validates the URL; guard here before it reaches the
