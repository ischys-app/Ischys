...
export async function recomputeForExercise(
  ...
  for (const rv of Object.values(computed) as RecordValue[]) {
    ...
    if (row) {
      await db.update(schema.personalRecords).set(fields).where(eq(schema.personalRecords.id, row.id));
    } else {
  ...
  for (const [metric, row] of byMetric) {
    if (!(metric in computed)) {
      await db.delete(schema.personalRecords).where(eq(schema.personalRecords.id, row.id));
    }
  }
  return computed;
}
