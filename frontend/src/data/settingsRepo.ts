/**
 * Settings on-device — local equivalent of /settings. A single settings row
 * (seeded on first run). Sync-related fields (sync_frequency, server_url) are
 * vestigial in the no-backend app and returned as inert defaults; Phase 1c
 * removes them from the UI. `triggerSync` is a no-op kept for callers.
 */
