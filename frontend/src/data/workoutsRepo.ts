/**
 * Workout lifecycle on-device — the local equivalents of the /workouts
 * endpoints. Reads assemble DTOs via ./queries + ./map; writes bump updated_at;
...
export async function finishWorkout(wid: string): Promise<WorkoutSummaryOut> {
  ...
  for (const eid of exerciseIds) baselines.set(eid, await currentValues(eid));
  ...
  for (const eid of exerciseIds) {
    const computed = await recomputeForExercise(eid);
    const deltas = detectPrs(baselines.get(eid) ?? {}, computed);
    for (const d of deltas) {
      if (d.value.workoutSetId && setIds.has(d.value.workoutSetId)) {
        await db.update(schema.workoutSets).set({ isPr: 1, updatedAt: nowMs() }).where(eq(schema.workoutSets.id, d.value.workoutSetId));
      }
    }
  ...
  await db.update(schema.workouts).set({ prCount: prs.length, updatedAt: nowMs() }).where(eq(schema.workouts.id, wid));
