 ...
 * catalog entry has the same id on every device — and, once the server adopts
 * the same scheme, the same id there too. Only then do cross-device references
 * to catalog rows line up without syncing the catalog itself. User-created rows
 * use a random id matching the server's `uuid4().hex` shape.
 *
...
/** Fresh random id for a user-created row (hex, matches server uuid4().hex). */
