/**
 * Pure volume & strength math — the TypeScript port of `backend/app/services/stats.py`,
 * kept byte-for-byte in sync so the local-first frontend and the server agree.
 *
...
// Python's round() is banker's rounding, but these values never land on a .5
// tie, so half-away-from-zero matches the server's numeric outputs exactly.
 ...
 *
 * Working sets only (warmups excluded), matching the PR definitions on the
 * server so the chart and the record cards agree:
