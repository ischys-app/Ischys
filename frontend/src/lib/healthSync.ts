...
// Matches the backend's HeartRateIn bounds (services/heart_rate.py). A stray
// sample outside this range is dropped rather than 400ing the upload.
const HR_MIN = 20;
const HR_MAX = 250;
...
export async function syncFinishedWorkout(
  ...
  try {
    ...
    // Upload HR aggregates to the server so the summary and history carry them.
    if (
      prefOn(hrPref) &&
      workoutId &&
      metrics.avgHr != null &&
